...
export default function ErrorPage({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#060914] px-4 text-slate-100">
      <section className="max-w-lg border border-slate-800 bg-slate-950 p-6">
        ...
        <div className="mt-5 flex gap-2">
          ...
          <button
            className="border border-slate-700 bg-slate-900 px-4 py-2 text-sm text-slate-200 hover:bg-slate-800"
            onClick={() => reset()}
            type="button"
          >
            重试
