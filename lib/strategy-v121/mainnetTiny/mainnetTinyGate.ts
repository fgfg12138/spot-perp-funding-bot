...
export function checkMainnetTinyGate(): MainnetTinyGateStatus {
  ...
  const mode = cfg.rawMode ?? "";
  const tinyEnabled = cfg.mainnetTiny.enabled ? "true" : undefined;
  const riskConfirmed = cfg.mainnetTiny.riskConfirmedRaw;
  const liveEnabled = cfg.controlledLive.enabled ? "true" : undefined;
