...
describe("marketRefreshService", () => {
  ...
  it("mocked refresh returns scan result with errors on network failure", async () => {
    ...
    expect(result.scanResult!.totalPaths).toBe(135); // V121_UNIVERSE × 3×3
  ...
  it("respects custom symbols list", async () => {
    ...
    // totalPaths is always universe size (135), not per-symbol
    expect(result.scanResult!.totalPaths).toBe(135);
  });
  ...
  it("handles empty symbols gracefully", async () => {
    ...
    // totalPaths still 135 (scanner uses full universe internally)
