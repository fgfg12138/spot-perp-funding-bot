...
async function discoverOkx(): Promise<SameExchangeUniverseItem[]> {
  ...
  // OKX SWAP 字段差异说明:
  //   - quoteCcy: 始终为空字符串（文档声明不适用）
  //   - baseCcy:  始终为空字符串（文档声明不适用）
  //   - settleCcy: "USDT" 表示 USDT 保证金（linear 正向合约）
  //   - uly: "BTC-USDT" 形式，从中提取 base 币种
  // 参考: https://www.okx.com/docs-v5/en/#rest-api-public-data-get-instruments
  const perpBases = new Set(
    (swapData.data ?? [])
      .filter((s: any) => s.settleCcy === "USDT" && s.state === "live")
      .map((s: any) => s.uly?.split("-")[0])
      .filter(Boolean),
  );
