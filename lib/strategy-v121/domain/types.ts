...
const ALLOWED_EXCHANGES: ExchangeId[] = ["binance", "okx"]; // HTX 已删除
...
export type CapitalPlanType = "conservative" | "moderate";
...
export interface CapitalPlan {
  ...
  spotRatio: number;
  /** 空单名义本金比例（如 0.30 = 30%） */
  shortNotionalRatio: number;
  /** 合约额外保证金缓冲（如 0.30 = 30%） */
  perpMarginBufferRatio: number;
  /** 备用现金比例（如 0.10 = 10%） */
  reserveRatio: number;

  // 计算值
  spotAmount: number;
  shortNotionalAmount: number;
  perpMarginBufferAmount: number;
  reserveAmount: number;
  // 合约账户总保证金 = shortNotionalAmount + perpMarginBufferAmount
  totalMarginAmount: number;
  // 可承受价格上涨幅度
  tolerablePriceRise: number;
}
