...
describe("opportunityCapabilityFilter — 本所套利", () => {
  ...
  it("OKX runtime 未支持时 blocked", () => {
    ...
    expect(result.executability).toBe("blocked");
    expect(result.blockers[0]).toContain("OKX");
    expect(result.blockers[0]).toContain("暂不支持");
  });
});
...
describe("opportunityCapabilityFilter — 跨所套利", () => {
  ...
  it("Binance + OKX 但 OKX not_supported 时 blocked", () => {
    const accounts = [
    ...
    expect(result.blockers.some((b) => b.includes("暂不支持"))).toBe(true);
  });
  ...
  it("跨所 crossExchangeArbEnabled=false 时 blocked（第一版默认关闭）", () => {
    ...
    const result = filterOpportunityByCapability(CROSS_PATH, accounts);
    expect(result.executability).toBe("blocked");
    // OKX not_supported + crossExchangeArbEnabled=false 都会阻塞
  ...
  it("跨所 perp side 权限不足时 blocked", () => {
    ...
    const result = filterOpportunityByCapability(CROSS_PATH, accounts);
    expect(result.executability).toBe("blocked");
    // OKX not_supported + perp 权限不足都会阻塞
...
describe("opportunityCapabilityFilter — 批量 + 聚合", () => {
  it("filterOpportunitiesByCapability 附加 capabilityResult", () => {
    ...
    expect(results[1].capabilityResult.executability).toBe("blocked"); // OKX not_supported
  });
