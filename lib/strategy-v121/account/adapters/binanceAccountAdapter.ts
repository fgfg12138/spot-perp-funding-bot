...
function makeFailed(leg: PlannedOrderLeg, error: string): ExchangeOrderSubmissionResult {
...
function dryRunResult(leg: PlannedOrderLeg, status: string): ExchangeOrderSubmissionResult {
...
function normalizeSpotResponse(response: any, leg: PlannedOrderLeg): ExchangeOrderSubmissionResult {
...
function normalizeFuturesResponse(response: any, leg: PlannedOrderLeg): ExchangeOrderSubmissionResult {
  const status = response.status ?? "UNKNOWN";
  return {
    ok: status !== "REJECTED",
...
function normalizeFuturesCloseResponse(response: any, leg: PlannedOrderLeg): ExchangeOrderSubmissionResult {
  const status = response.status ?? "UNKNOWN";
  return {
    ok: status !== "REJECTED",
...
function normalizeSpotCloseResponse(response: any, leg: PlannedOrderLeg): ExchangeOrderSubmissionResult {
  const status = response.status ?? "UNKNOWN";
  return {
    ok: status !== "REJECTED",
