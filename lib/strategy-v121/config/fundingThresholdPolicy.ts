 ...
 *
 * 生产阈值：0.05%（不可修改）
 * 测试阈值：V121_TEST_FUNDING_THRESHOLD_8H 环境变量（通过 runtimeConfig 读取）
 *
...
export function getFundingThreshold8h(context: FundingThresholdContext): {
  ...
  const enabled = cfg.testFundingThreshold.enabled;
  if (enabled && cfg.testFundingThreshold.value8h !== null) {
    const testVal = cfg.testFundingThreshold.value8h;
