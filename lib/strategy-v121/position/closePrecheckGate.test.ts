...
describe("closePrecheckGate", () => {
  ...
  it("blocks HTX spot exchange (not_supported)", () => {
    const result = runClosePrecheckGate({
      position: makePosition({
        path: { symbol: "BTC/USDT", spotExchange: "htx", perpExchange: "htx", isCrossExchange: false },
      }),
      exchangeSnapshot: makeSnapshot(),
    });
    expect(result.ok).toBe(false);
    expect(result.blockers.some((b) => b.includes("htx"))).toBe(true);
  });

  it("allows OKX same-exchange close", () => {
    const result = runClosePrecheckGate({
      position: makePosition({
        path: { symbol: "BTC/USDT", spotExchange: "okx", perpExchange: "okx", isCrossExchange: false },
      }),
      exchangeSnapshot: makeSnapshot({ exchange: "okx" }),
    });
    expect(result.ok).toBe(true);
  });
