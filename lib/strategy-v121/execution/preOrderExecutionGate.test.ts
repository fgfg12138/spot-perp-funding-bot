    ...
    if (ex === "binance") {
      return {
        adapter: {
          ...
          transferInternal: vi.fn().mockResolvedValue({ ok: true, txId: "mock" }),
          validateOrderPlan: vi.fn().mockResolvedValue({ ok: true, blockers: [], warnings: [] }),
          submitOrderLeg: vi.fn().mockResolvedValue({ ok: true, orderId: "mock", clientOrderId: "mock", status: "new" }),
          fetchOrderByClientOrderId: vi.fn().mockResolvedValue({ ok: true, orderId: "mock", status: "filled" }),
        },
        dataSource: "mock",
      };
    }
    return { adapter: { fetchOpenOrders: vi.fn().mockResolvedValue([]) }, dataSource: "mock" };
  };
  return { createAccountAdapter: createAdapter };
});
...
describe("runPreOrderExecutionGate", () => {
  ...
  it("3. kill switch active → blocked", async () => {
    ...
    const { resetRuntimeConfig } = await import("../config/runtimeConfig");
    resetRuntimeConfig();
    const r = await runPreOrderExecutionGate(baseInput);
