...
describe("executeGuardedTwoLegOrder", () => {
  ...
  it("4. exchange with no submitOrderLeg adapter → blocked", async () => {
    // 模拟一种 exchange 的 adapter 没有 submitOrderLeg（如仅观察的交易所）
    const { createAccountAdapter } = await import("../account/adapters/accountAdapterFactory");
    (createAccountAdapter as any).mockReturnValueOnce({
      adapter: { fetchOpenOrders: vi.fn().mockResolvedValue([]) },
      dataSource: "mock",
    });
    const { findOrderPlanById } = await import("./orderPlanLedger");
    (findOrderPlanById as any).mockResolvedValueOnce({ ...mockPlan, exchange: "htx" });
    const r = await executeGuardedTwoLegOrder(baseInput);
    expect(r.status).toBe("failed");
  ...
  it("8. kill switch → blocked", async () => {
    resetRuntimeConfig({ V121_KILL_SWITCH: "PAUSE_NEW_ENTRIES" });
    const r = await executeGuardedTwoLegOrder(baseInput);
